...
//
// Four sources produce this shape and are interchangeable everywhere downstream:
...
//   'recipe'  — the user's own dishes, via GET /api/recipes. Also browsed; its per-100g
//               values are derived from the recipe's totals and weight.
export interface FoodSearchResult extends Per100g {
  /**
   * Stable list key. An OFF barcode, `usda:<fdcId>` for a generic food, `custom:<uuid>`
   * or `recipe:<uuid>` for one of the user's own — prefixed because these are different
  ...
  nameBg?: string | null;
  brand: string | null;
  source: 'search' | 'generic' | 'custom' | 'recipe';
  /**
  ...
  /**
   * The recipe's id, for `source: 'recipe'` only. Same contract as `customFoodId`: it is
